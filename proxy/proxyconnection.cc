...
void ProxyConnection::addrFixupAndHonorRequest()
{
...
void ProxyConnection::honorRequest()
{
	try
	{
		switch (request->code)
...
void ProxyConnection::honorConnect()
{
	S6U::SocketAddress sockAddr(addr, request->port);
		
	upstream.dstSock.fd.assign(socket(sockAddr.sockAddress.sa_family, SOCK_STREAM | SOCK_NONBLOCK, IPPROTO_TCP));
	if (upstream.dstSock.fd < 0)
...
void ProxyConnection::honorConnectStackOptions()
{
	tfoPayload = std::min((size_t)request->options.stack.tfo.get().value_or(0), MSS);
}

void ProxyConnection::populateConnectStackOptions()
{
	if (S6U::Socket::hasMPTCP(upstream.dstSock.fd) > 0)
		reply.options.stack.mp.set(SOCKS6_STACK_LEG_PROXY_REMOTE, SOCKS6_MP_AVAILABLE);
}

ProxyConnection::ProxyConnection(Proxy *proxy, UniqFD &&srcFD)
	: FDXStreamReactor(proxy->getPoller()), proxy(proxy)
{
	upstream.srcSock.fd = move(srcFD);
	upstream.srcSock.keepAlive();
	
	TLSContext *serverCtx = proxy->getServerCtx();
	if (serverCtx)
		upstream.srcSock.tls = make_shared<TLS>(serverCtx, upstream.srcSock.fd);
}
...
void ProxyConnection::upstreamProcess(int fd, uint32_t events)
{
	switch ((State)state)
	{
	case S_READING_REQ:
	{
		ssize_t bytes = upstream.srcSock.sockRecv(&upstream.buf);
		if (bytes == 0)
			return;

		S6M::ByteBuffer bb(upstream.buf.getHead(), upstream.buf.usedSize());
		try
		{
			request = std::make_shared<S6M::Request>(&bb);
			upstream.buf.unuse(bb.getUsed());
		}
...
void ProxyConnection::resolvDone(optional<S6M::Address> resolved)
{
	if (!resolved)
	{
